...
try:
    from com.rackspace.cloud.servers.api.client.tests.account \
        import RS_UN, RS_KEY
except ImportError:
    sys.stderr.write(open(os.path.join(os.path.dirname(__file__), \
                          "README.txt")).read())
    sys.exit(1)
...
#----------------------------------------------------------------------------
from com.rackspace.cloud.servers.api.client.cloudserversservice \
    import CloudServersService
